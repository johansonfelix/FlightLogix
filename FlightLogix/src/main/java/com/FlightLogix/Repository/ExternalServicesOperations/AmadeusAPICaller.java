package com.FlightLogix.Repository.ExternalServicesOperations;
...
public class AmadeusAPICaller {
    private static final AmadeusAPICaller amadeusAPICaller = new AmadeusAPICaller();
    ...
    private void getProperties (String fileName){
        ...
        try {
            ...
            FileInputStream fis = new FileInputStream(new File("").getAbsolutePath()+"\\"+fileName);
            prop.load(fis);
            flightSearchURL = new URL(prop.getProperty("flightSearchURL"));
            tokenGenerationURL = new URL(prop.getProperty("tokenGenerationURL"));
